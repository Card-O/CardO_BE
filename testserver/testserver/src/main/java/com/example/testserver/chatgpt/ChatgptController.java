...
public class ChatgptController {
    ...
    public ChatgptController(ChatgptService chatGptService, ImageGenerationService imageGenerationService, ImageService imageService, UserRepository userRepository, ImagePromptRepository imagePromptRepository) {
    ...
    @PostMapping
    public Mono<String> chat(@RequestBody Map<String, String> data,WebSession session) {
                ...
                .flatMap(securityContext -> {
                    ...
                    return chatGptService.getChatGptResponse(promotionPrompt)
                            .flatMap(promotionText -> {
                                ...
                                // 비동기 작업 수행
                                performAsyncTask(translationPrompt, atmosphere, securityContext,session) // SecurityContext를 매개변수로 전달
                                        .subscribe(
                                                result -> {
    ...
    public Mono<String> performAsyncTask(String translationPrompt, String atmosphere, SecurityContext securityContext,WebSession session) {
                    ...
                    return chatGptService.getChatGptResponse(translationPrompt)
                            .flatMap(translatedText -> {
                                ...
                                if (translatedText != null) {
                                    ...
                                    imagePromptRepository.saveImagePrompt(userId,imagePrompt).subscribe();
                                    ...
                                    return imageGenerationService.generateImage(imagePrompt)
                                            .flatMap(imageUrls -> {
                                                ...
                                                return imageService.saveImageUrls(imageUrls, securityContext)
                                                        .doOnSuccess(savedUser -> System.out.println("저장된 사용자 정보: " + savedUser))
